...
namespace BindingTest
{
  ...
  public class BindingExpressionParserTest
  {
    ...
    public void Test1()
    {
      ...
      var expression = BindingExpression.ParseExpression( evaluator, "{binding path=Name}" );
      Assert.AreEqual( expression.Name, "binding", "测试解析绑定表达式失败" );
      ...
      expression = BindingExpression.ParseExpression( evaluator, "{binding path=Name , key==@#$}" );
      Assert.AreEqual( expression.Name, "binding", "测试解析绑定表达式失败" );
      Assert.IsTrue( expression.Arguments.ContainsKey( "path" ), "测试解析绑定表达式失败" );
      Assert.AreEqual( expression.Arguments["path"], "Name ", "测试解析绑定表达式失败" );
      ...
      expression = BindingExpression.ParseExpression( evaluator, "{binding path={{abc,,}},key=abc}" );
      Assert.AreEqual( expression.Name, "binding", "测试解析绑定表达式失败" );
      Assert.IsTrue( expression.Arguments.ContainsKey( "path" ), "测试解析绑定表达式失败" );
      ...
      expression = BindingExpression.ParseExpression( evaluator, "{binding path={},,key=abc}" );
      Assert.IsNull( expression, "错误的解析了错误的表达式" );

      expression = BindingExpression.ParseExpression( evaluator, "{binding path={binding a=b, c=d},key=abc}" );
      Assert.AreEqual( expression.Name, "binding", "测试解析绑定表达式失败" );
      Assert.IsTrue( expression.Arguments.ContainsKey( "path" ), "测试解析绑定表达式失败" );
