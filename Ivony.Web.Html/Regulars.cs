...
namespace Ivony.Web.Html
{
  public static class Regulars
  {
    ...
    public static readonly string pseudoClassPattern = string.Format( @":(?<name>[\w-]+)\p{{Zs}}*(\((?<args>{0}|[^)]*)\))?", quoteTextPattern );
    public static readonly string pseudoClassPatternNoGroup = string.Format( @":([\w-]+)\p{{Zs}}*(\(({0}|[^)]*)\))?", quoteTextPattern );
